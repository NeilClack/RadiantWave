...
func (p *ScreenOrientation) Init(app ApplicationInterface) error {
	var err error
	...
	p.title, err = NewStringItem("Screen Orientation", p.font, colors.White)
	if err != nil {
		return err
	}
	...
	dir := filepath.Join(config.AssetsDir, "affirmations")
...
func (p *ScreenOrientation) layout() {
...
func (p *ScreenOrientation) HandleEvent(event *sdl.Event) error {
	switch e := (*event).(type) {
...
func (p *ScreenOrientation) Render() error {
	...
	p.RenderSolidColorQuad(p.solidShader, p.affirmations[p.highlightIndex].Position(), p.affirmations[p.highlightIndex].RawDimensions(), colors.White)
...
func (p *ScreenOrientation) Destroy() error {
